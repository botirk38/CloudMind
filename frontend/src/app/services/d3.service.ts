...
export class D3Service {
  ...
  drawLineToMessage(
    ...
    if (isAbove) {
      ...
      startX =
        buttonClicked === 'left-button'
          ? textBoxPosition.x + textBoxSize.width
          : textBoxPosition.x;
    ...
    let endX =
      buttonClicked == 'left-button'
        ? newPosition.x + averageMessageSize.x
        : newPosition.x;
  ...
  drawLineToChild(
    ...
    const startX =
      buttonClicked === 'left-button'
        ? parentPosition.x + parentSize.x
        : parentPosition.x;
    const startY = parentPosition.y + parentSize.y / 2;

    const endX =
      buttonClicked === 'left-button'
        ? childPosition.x
        : childPosition.x + childSize.x;
    const endY = childPosition.y + childSize.y / 2;
